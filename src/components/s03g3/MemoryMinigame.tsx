    ...
    <section id="finale" className="scroll-mt-16 px-[clamp(10px,4vw,28px)] pt-16 pb-20 text-center">
      ...
      <div className="relative mx-auto max-w-[1024px]">
        ...
        >
          <div className="relative overflow-hidden rounded-[14px] bg-[#0a0f0a] p-[clamp(6px,2vw,12px)] sm:p-4" style={{ boxShadow: 'inset 0 0 50px rgba(0,0,0,.9),inset 0 0 0 3px #1a1a14' }}>
            <div className="relative animate-s03g3-flicker overflow-hidden rounded-lg px-[clamp(6px,3vw,14px)] pt-[20px] pb-[16px] sm:px-[20px]" style={{ background: 'radial-gradient(120% 120% at 50% 40%,#04140a,#020802)' }}>
              ...
              <div className="pointer-events-none absolute inset-x-0 z-20 h-[60px] animate-s03g3-scanline" style={{ background: isDistressed ? 'linear-gradient(180deg,transparent,rgba(230,57,70,.1),transparent)' : 'linear-gradient(180deg,transparent,rgba(51,255,102,.06),transparent)' }} />
              {isDistressed && (
              ...
              <div className="mb-2.5 flex flex-wrap items-center justify-center gap-1.5">
                {[
                  ...
                  <span
                    key={pill.label}
                    className={`rounded-full border px-2.5 py-[3px] font-s03g3-mono text-[10px] tracking-[.1em] ${pill.ok ? 'border-s03g3-crt/40 text-s03g3-crt/80 bg-s03g3-crt/5' : 'border-s03g3-alert/50 text-s03g3-alert bg-s03g3-alert/10'}`}
                  >
                    {pill.label}
                  </span>
                ))}
              ...
              {gameState === 'ready' && (
                <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#020802]/85 px-[20px] pb-4 backdrop-blur-[3px]">
                  <div className="font-s03g3-display text-[clamp(28px,5.5vw,46px)] font-bold uppercase tracking-[.1em] text-s03g3-crt" style={{ animation: 'matrixGlow 2s ease-in-out infinite' }}>
                    FDS Memory Rescue
                  </div>
                  <div className="mx-auto mt-4 max-w-[500px] font-s03g3-mono text-[15px] leading-relaxed text-s03g3-crt/80 text-left sm:text-[16px]">
                    <p>&gt; UPLINK ESTABLISHED: <span className="text-s03g3-crt">VOYAGER 1</span></p>
                    <p>&gt; DISTANCE: <span className="text-s03g3-crt">15 BILLION MILES</span></p>
                    <p>&gt; STATUS: <span className="text-s03g3-alert animate-pulse">MEMORY CORRUPTION DETECTED</span></p>
                    <br/>
                    <p>
                  ...
                  <button
                    ...
                    className="mt-8 animate-pulse rounded border border-s03g3-crt bg-s03g3-crt/10 px-9 py-3.5 font-s03g3-display text-[18px] font-bold tracking-widest text-s03g3-crt shadow-[0_0_15px_rgba(51,255,102,.2)] hover:bg-s03g3-crt hover:text-[#0a0f0a] hover:shadow-[0_0_25px_rgba(51,255,102,.6)]"
                  >
                    INITIALIZE RECOVERY
                  </button>
                </div>
              ...
              {gameState === 'success' ? (
                    <>
                      <div className="font-s03g3-display text-[clamp(22px,4vw,34px)] font-bold uppercase tracking-[.08em] text-s03g3-crt" style={{ animation: 'matrixGlow 1.6s ease-in-out infinite' }}>
                    Mission Complete
                  </div>
                  <p className="mx-auto mb-3 mt-2 max-w-[420px] font-s03g3-mono text-[11px] leading-relaxed text-s03g3-crt/70">
                ...
                <>
                  <div className="font-s03g3-display text-[clamp(22px,4vw,34px)] font-bold uppercase tracking-[.08em] text-s03g3-alert" style={{ textShadow: '0 0 14px rgba(230,57,70,.7)' }}>SIGNAL LOST</div>
                  <p className="mx-auto mb-3 mt-2 max-w-[380px] font-s03g3-mono text-[11px] leading-relaxed text-s03g3-alert/70">
                ...
                <>
                  <div className="font-s03g3-display text-[clamp(16px,3vw,22px)] font-bold uppercase tracking-[.08em] text-s03g3-crt" style={{ textShadow: '0 0 14px rgba(51,255,102,.7)' }}>Transmitting…</div>
                  <p className="mx-auto mb-3 mt-1 max-w-[420px] font-s03g3-mono text-[10px] leading-relaxed text-s03g3-crt/70">
                    Board locked. The patch is racing across 22 light-hours of space — radiation can still knock a block loose before it lands.
                  </p>
                </>
              ) : (
                <>
                  <div className="font-s03g3-display text-[clamp(16px,3vw,35px)] font-bold uppercase tracking-[.08em] text-s03g3-crt" style={{ textShadow: '0 0 14px rgba(51,255,102,.7)' }}>FDS MEMORY RESCUE</div>
                  <p className="mx-auto mb-3 mt-1 max-w-[500px] font-s03g3-mono text-[12px] leading-relaxed text-s03g3-crt/70">
                  Drag all ten blocks onto free addresses before the signal strength hits 0%. Avoid corrupted cells and spreading radiation.
                  </p>
                </>
              )}

              <div className="w-full h-1 bg-[#1a1a14] mb-3 overflow-hidden border border-s03g3-crt/20">
                <div
                  className={`h-full transition-all duration-700 ${signalStrength < 30 ? 'bg-s03g3-alert' : 'bg-s03g3-crt'}`}
                  style={{ width: `${signalStrength}%` }}
                />
              </div>

              <div className="mmg-board mx-auto grid w-full max-w-[920px] grid-cols-1 gap-3 md:grid-cols-[150px_1fr] md:gap-4">
                <div style={{ gridArea: 'tray' }} className="relative flex w-full flex-col gap-1 overflow-visible rounded-lg border border-s03g3-alert/40 bg-[linear-gradient(160deg,rgba(230,57,70,.08),rgba(0,0,0,.3))] p-1.5 md:gap-2 md:p-3">
                  ...
                  <div className="relative flex flex-col items-center justify-between gap-1 text-center">
                    <p className="glitch-label m-0 font-s03g3-mono text-[10px] font-bold tracking-[.06em] text-s03g3-alert md:hidden">
                      CORRUPTED BLOCKS
                    </p>
                    <p className="mb-0 pt-0 glitch-label hidden font-s03g3-mono text-[15px] font-bold text-s03g3-alert md:block">
                      CORRUPTED MEMORY
                    </p>
                    <p className="mt-0 pt-0 glitch-label hidden -mt-4 font-s03g3-mono text-[25px] font-bold text-s03g3-alert md:block">
                      BLOCKS
                    </p>
                  </div>
                    ...
                    {gameState === 'playing' && tray.length > 0 ? (
                      tray.map((b, i) => (
                        ...
                        >
                          ...
                          <span className="font-s03g3-mono text-[11px] font-bold tracking-wide md:text-[15px]" style={{ color: b.color }}>{b.label}</span>
                          <span className="hidden font-s03g3-mono text-[10px] tracking-[.08em] text-white/500 md:block">{b.shape.length} CELL{b.shape.length !== 1 ? 'S' : ''}</span>
                        </div>
                      ))
                    ) : gameState === 'playing' && allDone ? (
                      <button
                        onClick={handleTransmit}
                        className="flex min-h-[40px] animate-pulse items-center justify-center rounded border-2 border-s03g3-crt bg-s03g3-crt px-5 font-s03g3-display text-[12px] font-bold text-[#0a0f0a] hover:bg-[#aaffaa]"
                      >
                        TRANSMIT PATCH
                      </button>
                    ) : (
                      <p className="font-s03g3-mono text-[12px] text-s03g3-crt/40 mt-4 text-center">
                ...
                <div style={{ gridArea: 'grid' }} className="w-full min-w-0 overflow-hidden rounded-lg border border-s03g3-crt/25 bg-black/40 p-3 shadow-2xl backdrop-blur-md">
                <p className="mb-2 mt-0 text-left font-s03g3-mono text-[12px] tracking-[.14em] text-s03g3-crt/60">DESTINATION: HEALTHY MEMORY BANK — {dims.cols}×{dims.rows}</p>
                <div className="grid gap-[3px] sm:gap-[5px]" style={{ gridTemplateColumns: `repeat(${dims.cols}, minmax(0,1fr))` }}>
                  {Array.from({ length: SLOTS }).map((_, slot) => {
                    ...
                    if (isWarning) {
                      ...
                      cellClass += previewCells?.valid ? ' !border-solid !border-s03g3-crt bg-s03g3-crt/25' : ' !border-solid !border-s03g3-alert bg-s03g3-alert/25';
                      addressColor = 'rgba(255,255,255,.65)';
                    } else if (isCorrupt || isRadiation) {
                      cellClass += ' !border-dashed !border-s03g3-alert bg-[repeating-linear-gradient(45deg,transparent,transparent_2px,rgba(230,57,70,.15)_2px,rgba(230,57,70,.15)_5px)] shadow-[0_0_8px_rgba(230,57,70,.4)]';
                      content = isCorrupt ? '✕' : '☢';
                      ...
                      cellClass += ' border-s03g3-crt/20 bg-s03g3-crt/[0.03]';
                    }
                    ...
                    return (
                      <div key={slot} data-slot={slot} className={cellClass} style={extraStyle}>
                        {showAddress && (
                          <span
                            className="pointer-events-none absolute left-[3px] top-[2px] font-s03g3-mono font-bold leading-none"
                            style={{ fontSize: 'clamp(6px, 2.2vw, 10px)', color: addressColor, textShadow: occupant ? '0 1px 2px rgba(0,0,0,.6)' : 'none' }}
                          >
                            {toHex(slot)}
                          </span>
                        )}
                        <span className="font-s03g3-mono text-[clamp(16px,5vw,40px)] text-s03g3-alert">{content}</span>
                      </div>
                    );
                  })}
                </div>
                  ...
                  <div style={{ gridArea: 'next' }} className="relative w-full flex flex-wrap items-center justify-between gap-2 rounded-md border border-s03g3-crt/20 bg-black/30 px-3 py-2 shadow-xl backdrop-blur-md">
                    <div className="flex items-center gap-3">
                      <span className="font-s03g3-mono text-[14px] font-bold tracking-[.1em] text-s03g3-crt/70">NEXT UP: </span>
                      {nextUp.length > 0 ? (
                        ...
                        <span className="font-s03g3-mono text-[10px] text-s03g3-crt/40">— NONE —</span>
                      )}
                    </div>
                    <span className="whitespace-nowrap rounded-full border border-s03g3-crt/40 bg-s03g3-crt/10 px-3 py-1 font-s03g3-mono text-[11px] font-bold tracking-[.08em] text-s03g3-crt">
                      {remainingToPlace} BLOCK{remainingToPlace !== 1 ? 'S' : ''} LEFT
                    </span>
                  </div>
              </div>

              <div className="mx-auto mt-2.5 flex w-full max-w-[920px] items-center gap-2 rounded-full border border-s03g3-crt/25 bg-black/40 px-3 py-2">
                <span className={`flex-1 truncate text-left font-s03g3-mono text-[12px] ${gameState === 'fail' ? 'text-s03g3-alert' : gameState === 'success' ? 'text-green-300' : 'text-s03g3-crt'}`}>
                  &gt; {logMessage}<span className="animate-s03g3-blink">█</span>
                </span>
                {gameState !== 'ready' && (placedCount > 0 || gameState !== 'playing') ? (
                  <button
                    onClick={reset}
                    className="flex min-h-[40px] shrink-0 items-center justify-center rounded-full border border-s03g3-alert/60 bg-s03g3-alert/15 px-4 font-s03g3-mono text-[10px] font-bold tracking-[.05em] text-s03g3-alert hover:bg-s03g3-alert/25"
                  >
                    ⟲ RESET
                  </button>
                ) : null}
          ...
          <div className="flex items-center justify-between px-2 pt-3.5 pb-1">
            <span className="font-s03g3-display text-[12px] tracking-[.1em] text-[#8a8470]">ROG-6700</span>
            <span className="flex gap-2.5">
              <span className={`h-2.5 w-2.5 rounded-full ${gameState === 'success' ? 'bg-s03g3-crt shadow-[0_0_8px_#33ff66]' : gameState === 'fail' ? 'bg-s03g3-alert shadow-[0_0_8px_#e63946]' : 'bg-[#5a5544]'}`} />
      ...
      {drag && (
        ...
        >
          ...
          <span className="font-s03g3-mono text-[8px] font-bold tracking-wide" style={{ color: drag.color }}>{drag.label}</span>
        </div>
      )}
    </section>
  );
