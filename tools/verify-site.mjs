...
export function verifySite(distDir, exhibits) {
  ...
  for (const e of exhibits) {
    if (e.status === 'external') {
      ...
      if (!existsSync(route)) errors.push(`external exhibit ${e.slug} has no embedded output at ${route}`);
      continue;
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const verified = exhibits.filter((e) => e.status === 'live' || e.status === 'external').length;
  for (const error of errors) console.error(`FAIL ${error}`);
  console.log(`${verified}/53 exhibits live, ${errors.length} problems`);
  process.exit(ok ? 0 : 1);
}
